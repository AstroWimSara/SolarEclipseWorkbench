...
- **take_bracket** - Set the aperture, shutter speed and ISO of the camera and take a bracket of 5 pictures with the given steps. Make sure to have 5 steps enabled for bracketing (note - if not supported, it will fallback to 3 steps instead). Options for the steps are: +/- 1/3, +/- 2/3, +/- 1, +/- 1 1/3, +/- 1 2/3, +/- 2, +/- 2 1/3, +/- 2 2/3, +/- 3. This syntax only works for Canon. Sony doesn't support it, so instead you have to provide the desired mode name in this field instead (e.g. `"Bracketing C 3.0 Steps 5 Pictures"`). You can look up for the available modes using this command: `gphoto2  --get-config=/main/capturesettings/capturemode`. As for Nikon - it also doesn't support this syntax, so please use instead this convention: `"0.7 5"`. First float number is exposure step in EV and the other number is the amount of frames to take in the bracketing sequence.  An important note for Nikon cameras - app will assume you already chose Continuous mode (please see `take_burst` above for more information).
